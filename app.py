...
if uploaded:
    ...
    if 'df' not in st.session_state or current_file_id != previous_file_id:
        ...
        # Показываем сообщение о загрузке нового файла
        st.success(f"Новый файл '{uploaded.name}' успешно загружен!")
...
# ~~~~~~~~~~~~~~~~~~~~~ Основная информация ~~~~~~~~~~~~~~~~~~~~~
    st.subheader("Основная информация о датасете")
...
# ~~~~~~~~~~~~~~~~~~~~~ Визуализация пропусков ~~~~~~~~~~~~~~~~~~~~~
    ...
    if missing_row_indices:
        ...
        st.info("В данных нет пропущенных значений.")
...
# ~~~~~~~~~~~~~~~~~~~~~ Анализ выбросов ~~~~~~~~~~~~~~~~~~~~~
    ...
    if len(numeric_cols) > 0:
        ...
        if total_outliers > 0:
            ...
            # Выбросов нет — показываем статус
        ...
        st.info(f"Метод: замена значений за пределами IQR на границы (только для признака {outlier_col})")
